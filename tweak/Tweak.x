// Andante, Modified by idoit_Melon from Skitty
...
#import "SpringBoard+Andante.h"
#import <rootless.h>

#define BUNDLE_ID @"com.idoitmelon.andante"
...
// static BOOL enabledInApp = NO; // whether enable in music app
...
static void refreshPrefs() {
	...
	[ANChatManager sharedManager].window.rootViewController.shouldPlayFeedback = YES;
	// location : banner location
	// [ANChatManager sharedManager].window.rootViewController.location = [([settings objectForKey:@"location"] ?: @(0)) intValue];
	// [ANChatManager sharedManager].window.rootViewController.disableAutoDismiss = [([settings objectForKey:@"disableAutoDismiss"] ?: @(NO)) boolValue];
	// [ANChatManager sharedManager].window.rootViewController.dismissWhenExpanded = [([settings objectForKey:@"dismissWhenExpanded"] ?: @(NO)) boolValue];
	[ANChatManager sharedManager].window.rootViewController.bannerView.matchSystemTheme = [([settings objectForKey:@"matchSystemTheme"] ?: @(YES)) boolValue];
	[ANChatManager sharedManager].window.rootViewController.bannerView.darkMode = [([settings objectForKey:@"darkMode"] ?: @(YES)) boolValue];
	// [ANChatManager sharedManager].window.rootViewController.bannerView.oled = [([settings objectForKey:@"oled"] ?: @(NO)) boolValue];
	// [ANChatManager sharedManager].window.rootViewController.bannerView.blurred = [([settings objectForKey:@"blurred"] ?: @(NO)) boolValue];
	// [ANChatManager sharedManager].window.rootViewController.bannerView.blurThickness = [([settings objectForKey:@"blurThickness"] ?: @(1)) intValue];
	[ANChatManager sharedManager].window.rootViewController.bannerView.tintStrength = [([settings objectForKey:@"tint"] ?: @(NO)) boolValue] ? [([settings objectForKey:@"tintStrength"] ?: @(0.3)) floatValue] : 0;
	[[ANChatManager sharedManager].window.rootViewController.bannerView updateColors];
	// [ANChatManager sharedManager].window.rootViewController.bannerView.showBannerArt = [([settings objectForKey:@"showBannerArt"] ?: @(NO)) boolValue];

	// BOOL customDismissInterval = [([settings objectForKey:@"customDismissInterval"] ?: @(NO)) boolValue];
	// [ANChatManager sharedManager].window.rootViewController.dismissInterval = customDismissInterval ? [([settings objectForKey:@"dismissInterval"] ?: @(3)) intValue] : 3;
}

// static void PreferencesChangedCallback(CFNotificationCenterRef center, void *observer, CFStringRef name, const void *object, CFDictionaryRef userInfo) {
	// refreshPrefs(); // temporarily disabled
// }

// Create now playing window
%hook SpringBoard
%property (nonatomic, retain) ANChatManager *andanteManager;

- (void)applicationDidFinishLaunching:(id)arg1 {
	%orig;

	self.andanteManager = [ANChatManager sharedManager];
	self.andanteManager.enabled = enabled;
	if (@available(iOS 16.0, *)) {
		...
		self.andanteManager.window = [[%c(ANChatWindow) alloc] initWithWindowScene:windowScene role:nil debugName:@"AndanteWindow"];
	} else if (@available(iOS 15.0, *)) {
		self.andanteManager.window = [[%c(ANChatWindow) alloc] initWithScreen:[UIScreen mainScreen] role:nil debugName:@"AndanteWindow"];
	} else {
		self.andanteManager.window = [[%c(ANChatWindow) alloc] initWithScreen:[UIScreen mainScreen] debugName:@"AndanteWindow"];
	}
	self.andanteManager.window.rootViewController = [[ANChatViewController alloc] init];

	[self addActiveOrientationObserver:self.andanteManager];
	// if (@available(iOS 16.0, *)) {
...
// 	%orig;
// 	if (app) [[ANChatManager sharedManager].window.rootViewController appPlayingUpdate:app.bundleIdentifier];
// }
...
- (void)singlePress:(id)arg1 {
	%orig;
	[[ANChatManager sharedManager].window.rootViewController animateOut];
}

%end

// Disable reachability when banner is presented from the bottom
%hook SBReachabilityManager // reachability is this:https://www.idownloadblog.com/2017/11/07/how-to-use-reachability-iphone-x/

- (void)toggleReachability {
	if ([ANChatManager sharedManager].enabled == YES && [ANChatManager sharedManager].window.hidden == NO && [ANChatManager sharedManager].window.rootViewController.location == 1) return;
	%orig;
}
...
// 			}
// 			if (bundleIdentifier != nil) {
// 				[[ANChatManager sharedManager].window.rootViewController appPlayingUpdate:bundleIdentifier];
// 			}
// 		}
// 	});
// }
...
// 		[[ANChatManager sharedManager].window.rootViewController nowPlayingUpdate:userInfo];
// 	});	
// }
...
// %ctor {
	...
	// refreshPrefs(); // temporaryly disabled
